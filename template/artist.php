...
if(empty($_GET['artistKeyName']))
{
	echo '<script>window.location = "'.URL.'/#/start"</script>';
	die();
}
else
{
	$artistKeyName = $_GET['artistKeyName'];
	if($db->query('SELECT id FROM artists WHERE keyname="'.$_GET['artistKeyName'].'"')->num_rows > 0)
	{
		$artistOnDb = $db->query('SELECT id,name FROM artists WHERE keyname="'.$_GET['artistKeyName'].'"')->fetch_row();
		$artistId = $artistOnDb[0];
		$artistName = $artistOnDb[1];
	}
}
?>
<div data-ng-controller="ArtistCtrl as artist">
  ...
  <div class="artist-sub-header">
	...
	{
		if($db->query('SELECT id FROM usersFollowingArtist WHERE userId="'.$userId.'" AND artistId="'.$artistId.'"')->num_rows == 0)
		{
			$pseudoName = 'notfollowing';
			$pseudoButton = 'btn btn-default';
			$pseudoStatus = null;
		}
		else
		{
			$pseudoName = 'following';
			$pseudoButton = 'btn btn-primary';
			$pseudoStatus = null;
		}
	}
	else
	{
		$pseudoName = 'buttondisabled';
		$pseudoButton = 'btn btn-info';
		$pseudoStatus = ' disabled';
	}
	?>
	<button id="followButtonArtist" class="<?php echo $pseudoButton ?>" data-ng-click="artist.followtoggle(<?php echo $artistId ?>)"<?php echo $pseudoStatus ?>><?php echo str_replace('{name}',$artistName,$lang['artist.follow.'.$pseudoName]) ?></button>
    </div>
  </div>
          ...
          <div class="panel-body">
            ...
            <div>
              ...
              <ul class="list-inline">
                <li>
                  <img src="<?php echo URL ?>/style/images/profile/default.png" alt="" class="img30_30 img-circle" tooltip-placement="bottom" tooltip="John Doeser" tooltip-append-to-body="true">
                </li>
                <li>
                  <img src="<?php echo URL ?>/style/images/profile/default.png" alt="" class="img30_30 img-circle" tooltip-placement="bottom" tooltip="John Doeser" tooltip-append-to-body="true">
                </li>
            ...
            <div class="related-items">
              ...
              <div class="related-item">
                <div class="item-image">
                  <img src="<?php echo URL ?>/style/images/profile/default.png" class="img40_40"/>
                </div>
                <div class="item-name">
                  <h5>
                    The Beatles
